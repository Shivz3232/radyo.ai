    ...
    useEffect(()=>{
      const timer= setInterval(()=>{
        const handleChange=async()=>{
          ...
          setData(arrayData);
          const mappedData=[]
          const otherArray= arrayData.sort((a,b)=>a.value<b.value?1:-1).slice(0,10);
          otherArray.map((item,index)=>{
           const id=index;
           const title =item.creatorName;
           const value=getCreatorScore(item.playCount, item.audiosPublished,item.subscriberCount);
